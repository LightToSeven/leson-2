    ...
    <div class="app">
        ...
        <div class="app__list">
            ...
            <div class="app__item--wrap">
                ...
                <div class="app__item--favorites">
                    <list-item v-for='contact in filteredFavorites' :key='contact.id' :ref='"item-" + contact.id'
                               @delete-contact='fv_deleteContact' @edit-contact='editContact' @fv-contact='fvContacts_remov'
                               :contact='contact'>
                    </list-item>
                </div>
                <list-item v-for='contact in filteredContacts' :key='contact.id' :ref='"item-" + contact.id'
                           @delete-contact='deleteContact' @edit-contact='editContact' @fv-contact='fvContacts_add'
                           :contact='contact'>
                </list-item>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            favoritesList_limit: function () {
                localStorage.setItem('favoritesList_limit', JSON.stringify(this.favoritesList_limit))
            }
        },
        ...
        computed: {
            ...
            filteredFavorites() {
                ...
                const searchQ = this.searchQ.toLowerCase();
                const filteredFavorites = favoritesList.filter(el => {
                    return el.name.toLowerCase().indexOf(searchQ) !== -1;
                })
        ...
        methods: {
            ...
            fvContacts_add(contact) {
                if (this.favoritesList_limit<3) {
                    ...
                    this.favoritesList_limit++;
                }
                console.log(this.favoritesList_limit);
            },
            fvContacts_remov(contact) {
                if (this.favoritesList_limit>0) {
                    this.favoritesList_limit--;
                }
                this.contactsList.push(contact);
                for (const [index, value] of this.favoritesList.entries()) {
                    if (value.id == contact.id) {
        ...
        created() {
            ...
            const favoritesList_limit = JSON.parse(localStorage.getItem('favoritesList_limit'));
            if (contactsList) {
            ...
            if (favoritesList_limit) {
                this.favoritesList_limit = favoritesList_limit;
            }
        },
